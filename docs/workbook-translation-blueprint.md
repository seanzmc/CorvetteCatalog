...
This document brings together the source families, implemented destinations, runtime meanings, and the option-consolidation decision. The owner separately authorized implementation of the option slice. The earlier [Checkpoint A specification](source-schema-specification.md) supplies source-field detail and broader project planning; its proposed entity boundaries are not requirements to retain redundant tables.

## Start with an actual option

`stingray_options`, Excel row **128**, contains Z51 Performance Package:

| Workbook field | Actual value | Implemented owner |
|---|---|---|
...
This is a simplified view of the implemented option area. Rule tables additionally reference options and, where applicable, interiors. Typed foreign keys retain same-model constraints; polymorphic rule endpoints accept only options or model interiors.

## Complete source-family map

`option` means **a model's option**, identified by its model and workbook option ID. The current [importer](../catalog/importer.py) resolves the exact sheet names through `model_workbook_sources`; it does not infer them from spelling. The 66 assignments cover 11 roles for each of six models, including shared interior/color sheets.
...
| Each model's `options` role | `option` | Option identity, copy, base price, placement and selection flags |
...
**First structural change implemented:** one model-owned option row replaces the six-table split above. Availability and variant overrides remain separate relationships. Distinct rule families remain because they perform distinct operations. Workbook IDs remain model-qualified; RPO is not a unique product identity (`grandSport_options!74` and `!126` both use T0E with different behavior).
...
Schema 3 implements the option consolidation in the disposable candidate. All 1,379 option records and every other table were reconciled against schema 2 after translating internal option IDs and reference-column names. Source rows, source dispositions, code evidence, prices, relationships and ordering are preserved. Evidence links from the six former owners collapse to the single option owner. All six generated contracts and the registry retain frozen parity; see the [current implementation validation](../catalog/README.md#option-consolidation-schema-3).

This is **not complete model-specific behavioral coverage, final authoring DDL, or a claim that every remaining table boundary has passed ownership review**. The [model-specific review](model-rule-review.md) identifies concrete differences and untested obligations. Workbook data, runtime behavior, dependencies, hosting and synchronization remain unchanged. The known R6X pricing defect is preserved by this parity-only structural change.
