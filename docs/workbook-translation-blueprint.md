...
- **R6X requirement, clarified by the owner:** selecting a qualifying interior adds R6X. R6X never offsets the price of another item. The earlier description incorrectly presented existing seat/interior arithmetic as the R6X business rule. Current generator code adds a seat-rate difference for R6X-marked interiors; browser code separately subtracts selected-seat price and treats an `r6x` component as seat-replacing. Those implementation details require reconciliation against the clarified requirement, not automatic preservation. See the [R6X trace and model-specific coverage review](model-rule-review.md). No pricing code was changed by this correction.
