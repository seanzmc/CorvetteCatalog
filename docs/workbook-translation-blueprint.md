...
The workbook data and established form logic define this migration. The database structure must represent them correctly. Changing table boundaries, names, or relationships is allowed in the design; changing a price, applicability, rule meaning, or observable form behavior is a separate business change. There is no production catalog database to preserve at this stage.
...
- **Interior pricing**: generator uses exact-trim component rates with universal-trim fallback. R6X adds the nonnegative upgrade-seat minus base-seat difference to the stored interior amount. Browser subtracts the selected seat's resolved price and itemizes components without charging a replaced seat twice (`adjustedInteriorPrice`, line 795; `lineItemsFromInterior`, line 1309).
...
This is a complete map of current source families and behavior mechanisms, with an explicit first consolidation proposal. It is **not final DDL or a claim that every remaining table boundary has passed ownership review**. No application schema, workbook data, runtime behavior, dependencies, hosting or synchronization changed with this document.
